...
def submit_create_task():
    ...
    print(dicionario)
    specific_key = "message"
    if isinstance(dicionario, dict) and specific_key in dicionario and isinstance(dicionario[specific_key], str) and dicionario[specific_key] == "Task Successfully Added":

        return render_template('submit_create_task.html')
    else:
        # Mostra os dados retornados já convertidos
        return 'deu problema ai'
...
def see_tasks():
    ...
    if 'message' in dd:
        ...
        list_dict = [{'id': item['id'],'name': item['name'], 'realized': item['realized']} for item in data]
...
def delete_tasks_submit():
    ...
    specific_key = "message"
    if isinstance(dicionario, dict) and specific_key in dicionario and isinstance(dicionario[specific_key], str) and dicionario[specific_key] != "Task deleted Successfully":
...
def submit_update_task():
    ...
    dicionario = json.loads(dados.text)
    specific_key = "message"
    if isinstance(dicionario, dict) and specific_key in dicionario and isinstance(dicionario[specific_key], str) and dicionario[specific_key] != "Task Successfully Updated":
